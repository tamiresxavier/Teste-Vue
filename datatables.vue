<template>
    <div :class="classes">
        <table v-once :id="tableId" ref="table" :class="className" cellpadding="0">
            ...
            <tfoot v-if="!hideTfoot" :class="tfootClassName">
                <tr>
                    <th v-for="(field, i) in options.columns" :key="'foot_' + i" :class="field.classFooterName">
                        <slot :name="'FOOT_' + field.name" :field="field" :i="i">
                            <input
...
export default {
    ...
    mounted() {
        ...
        for (let k in cols) {
            const col = cols[k];
            if (col.template || that.$slots[col.name]) {
        ...
        if (that.details) {
            ...
            if (that.details.template || that.$slots['_details']) {
...
.vdtnet-footer .dataTables_length,
.vdtnet-footer .dataTables_paginate {
    float: right;
}
